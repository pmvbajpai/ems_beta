 ...
 *
 * The followings are the available columns in table 'tblstate':
...
class State extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('CountryId, StateName, DateCreated, DateUpdated', 'required'),
			array('CountryId', 'numerical', 'integerOnly'=>true),
			array('StateName', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('StateId, CountryId, StateName, DateCreated, DateUpdated', 'safe', 'on'=>'search'),
		);
	}
